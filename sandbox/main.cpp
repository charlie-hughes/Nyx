...
int main(int argc, const char** argv) {
    ...
    Nyx::Window window(1600, 900, "Sandbox");
    ...
    while (!window.GetWindowShouldClose()) {
        ...
        renderer.DrawQuad({3.0f, -1.5f}, {2.5f, 2.5f}, {0.2f, 0.2f, 0.6f, 1.0f}, rot); // Blue quad

        for (int y = 0; y < 27; y++) {
            for (int x = 0; x < 48; x++) {
                float colour_multiple = (x + y) % 2 + 2;
                renderer.DrawQuad({-24.5+x, -13+y}, {1.0f, 1.0f}, {0.2f*colour_multiple, 0.2f*colour_multiple, 0.2f*colour_multiple, 1.0f}, 0.0f, -0.1f);
            }
        }
